function [Q,time,acceptrate,ws,ess] = run_hmc(N,K,d,delta,T,pot,P,Uu)
%%%%%%%%%%%%
%N iterations
%K integration steps
%d dimension
...
%P random cars P randn(d, N)
%Uu random acceptance chance rand(1,N)



% Select potential
if pot == "dwell_lip" %F = (x+1)^2*(x-1)^2/(1+x^2)
    ...
    grad2U = @ (Delta,L,x) -L + spdiags( ...
        ...
        ./ ((x.^2 + 1).^6), ...
        0, numel(x), numel(x));
elseif pot == "dwell_nonlip" %F = x^4/4 - x^2/2 
...
%%%%%%%%%%%%%%%%%%%%%%%%% 
%% plotting vars
Q = NaN([N d]); %Final chain (storing whole chain)
%Q = NaN([N 3]); %Final chain (storing only 3 points)
iters = NaN([N 1]); %Newton iter counts
opt = NaN([N 1]); %Newton final norm(update) val
...
function [q_n,iters,opt] = newton(q0,p,delta,tol,I,Delta,L,gradU,Ip)
    ...
    for jn = 1:10^4
        ...
        if jn == 10^4 disp("fail");opt = norm(I(q_n,q0,delta,Delta,p,L),Inf); end %max iterations
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%
...
Q = [init_q';Q]; %adding initial q to start of the chain var
time = toc;

acceptrate = accept_count/N;

%Change coord to 3 if saving only first,mid,last points and vice versa
