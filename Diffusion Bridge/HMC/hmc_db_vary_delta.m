%Use this for multiple runs with different delta

N = 10^5; 
hn=10; %integrator steps per 1 N step
sims = 1; %# of total runs
d = 500;
tol = 10^-8;
delta = linspace(0.008,0.008,sims); %delta values
T = 10;
...
for s = 1:sims
    ...
    for j = 1:N
        q=q0;
        p0 = normrnd(0,1,([d 1])); p=p0;

        for hj = 1:hn
            [q1,iter_count,result] = newton(q,p,delta,tol,I,Delta,L,d,l2,F1,F2,F3,F4); %newton method to solve implicit step
            iters(j,s) = iter_count;
            opt(j,s) = result;
            p1 = p - sqrt(2*delta/Delta).*gradU(Delta,L,q1/2 + q/2); %p_n 
            q=q1;
            p=p1;
        end
